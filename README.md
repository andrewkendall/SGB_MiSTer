...
Speed and Mute are recognized by the SGB BIOS itself through the same button sequences the real Commander sent. Each state advances once per automatic controller read; the BIOS's shorter multitap-probe reads are ignored. They work with every SGB BIOS revision (SGB1 v1.0/1.1/1.2 and SGB2) and, like the real Commander, only operate from controller port 1.
