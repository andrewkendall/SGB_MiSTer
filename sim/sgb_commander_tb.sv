...
module sgb_commander_tb;
	...
	// The BIOS multitap check reads eight bits while the strobe is high.
	// It reads eight more bits after the strobe falls.
	// Neither part is a complete controller word.
	task multitap_probe;
	begin
		latch = 1;
		tick;
	...
	// The automatic SNES read shifts all 16 controller bits.
	task automatic_read;
	begin
		latch = 1;
		repeat (2) tick;
	...
	initial begin
		...
		// The SFC switch position passes all standard SNES inputs through.
		joy_in = 12'hfff;
		tick;
		expect_output(12'hfff);

		// In the SGB position, X is Color. R becomes L+R for Window.
		commander_en = 1;
		joy_in = 12'h040;
		tick;
		...
		// Y starts the exact three-mode Speed sequence.
		joy_in = 0;
		tick;
		joy_in = 12'h080;
		tick;
		// The module consumes the trigger while the request is pending.
		// A short BIOS probe cannot start the sequence.
		// The ninth clock of a full read starts step zero for the next read.
		// The ioport register already contains the current word.
		...
		// Release Y. Enable Dash, and press Y again.
		// The final sequence state is Y+Right.
		joy_in = 0;
		tick;
		dash_en = 1;
		joy_in = 12'h080;
		...
		// L starts the eight-state Mute sequence.
		joy_in = 0;
		tick;
		joy_in = 12'h100;
		tick;
		...
		// Set the switch to SFC. This change stops the command and restores L/Y.
		joy_in = 0;
		tick;
		joy_in = 12'h080;
		tick;
		...
		// Apply a trigger before the full-read marker.
		// The stored word must not use step zero.
		commander_en = 1;
		joy_in = 0;
		tick;
		latch = 1;
		...
		// Apply a trigger after the marker.
		// The trigger waits for the next full read.
		commander_en = 0;
		joy_in = 0;
		tick;
		commander_en = 1;
